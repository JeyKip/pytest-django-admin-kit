...
The default site registers Category, Feed and Product from the test project and User
and Group from `django.contrib.auth`, which gives two apps and five models.
"""
...
def test_a_superuser_sees_every_app_and_model_in_the_admins_order(admin_ui, superuser):
    ...
    assert page.models == [Group, User, Category, Feed, Product]
    assert page.models_for("Authentication and Authorization") == [Group, User]
    assert page.models_for("Shop") == [Category, Feed, Product]
